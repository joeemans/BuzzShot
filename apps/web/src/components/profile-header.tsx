        ...
        <div className="flex items-center gap-4">
          {profile.avatarUrl ? (
            <img
              ...
              width={80}
              height={80}
              className="h-20 w-20 rounded-md border border-border bg-white/10 object-cover"
            />
          ) : (
            <div className="flex h-20 w-20 items-center justify-center rounded-md border border-border bg-white/10 text-2xl font-black text-muted">
          ...
          <div>
            ...
            {profile.bio ? (
              <p className="mt-3 max-w-2xl text-sm leading-6 text-muted">{profile.bio}</p>
            ) : null}
          </div>
        </div>
        ...
        {Object.entries(profile.stats).map(([label, value]) => (
          <div key={label} className="rounded-md border border-border bg-black/25 p-3">
            ...
            <div className="text-xs uppercase text-muted">
              {label === 'watchlist' ? 'private watchlist' : label}
            </div>
          </div>
        ))}
      </div>
